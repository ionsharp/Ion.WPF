...
public class HorizontalGrid : Grid
{
    ...
    private static void OnItemsSourceChanged(DependencyObject i, DependencyPropertyChangedEventArgs e) => i.As<HorizontalGrid>().OnItemsSourceChanged(e.Convert<IEnumerable>());
    ...
    protected virtual GridSplitter OnSplitterCreated()
    {
        ...
        result.Bind
            ...
            ValueConverter.Cache.Get<ConvertBooleanToVisibility>());
        return result;
    }

    ///

    protected virtual void OnItemsSourceChanged(ValueChange<IEnumerable> input)
    {
        Clear();
        if (input.NewValue != null)
        {
            var children = input.NewValue.Cast<object>().Select(OnContainerCreated).ToArray();
