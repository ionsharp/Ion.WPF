...
public class XOrPolygon : XOrElement
{
    ...
    private static PathGeometry Render(ShapePoint shape)
    {
        var figure = new PathFigure();
        if (shape.Points?.Count > 0)
        {
            figure.StartPoint = new(shape.Points[0].X, shape.Points[0].Y);
    ...
    protected override void OnRender(DrawingContext drawingContext)
    {
        ...
        if (Points?.Count > 1)
        {
            var shape = new ShapePoint(Points.Select<Point, Vector2>(i => new Vector2(i.X, i.Y)));
            drawingContext.DrawGeometry(Brushes.Transparent, new Pen(Brushes.Black, StrokeThickness1 / Zoom) { DashStyle = DashStyle1 }, Render(shape));
            drawingContext.DrawGeometry(Brushes.Transparent, new Pen(Brushes.White, StrokeThickness2 / Zoom) { DashStyle = DashStyle2 }, Render(shape));
        }
    }
}
