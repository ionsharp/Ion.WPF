...
public static class XPanel
{
    ...
    private static void OnHorizontalContentAlignmentChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        if (sender is Panel panel)
            panel.AddHandlerAttached(true, HorizontalContentAlignmentProperty, HorizontalContentAlignment_Loaded, null);
    }

    private static void HorizontalContentAlignment_Loaded(Panel panel)
    {
        ...
        for (int i = 0, count = panel.Children.Count; i < count; i++)
        {
            ...
            if (XElement.GetOverrideHorizontalAlignment(j) is null)
    ...
    private static void OnVerticalContentAlignmentChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        if (sender is Panel panel)
            panel.AddHandlerAttached(true, VerticalContentAlignmentProperty, VerticalContentAlignment_Loaded, null);
    }

    private static void VerticalContentAlignment_Loaded(Panel panel)
    {
        var value = GetVerticalContentAlignment(panel);
        for (int i = 0, Count = panel.Children.Count; i < Count; i++)
        {
            var j = panel.Children[i] as FrameworkElement;
            if (XElement.GetOverrideVerticalAlignment(j) is null)
    ...
    private static void OnSpacingChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        if (sender is Panel panel)
            panel.AddHandlerAttached(true, SpacingProperty, Spacing_Loaded, null);
    }

    private static void Spacing_Loaded(Panel panel)
    {
        var spacing
            = GetSpacing(panel);
        var except
        ...
        {
            var j = panel.Children[i].As<FrameworkElement>();
            if (XElement.GetOverrideMargin(j) is null)
            {
                j.Margin
