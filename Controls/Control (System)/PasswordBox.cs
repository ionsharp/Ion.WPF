...
public static class XPasswordBox
{
    ...
    {
        if (sender is PasswordBox passwordBox)
            e.CanExecute = !passwordBox.Password.IsEmpty() && passwordBox.IsEnabled;
    }
    ...
    private static void OnGenerateCommandExecuted(object sender, ExecutedRoutedEventArgs e)
    {
        if (sender is PasswordBox i)
        {
            var result = GetGenerateCharacters(i).Random(GetGenerateLength(i).Minimum, GetGenerateLength(i).Maximum);
            result = GetGenerateDistinct(i) ? string.Concat(result.Distinct()) : result;
            i.Password = result;
        }
    }

    private static void OnGenerateCommandCanExecute(object sender, CanExecuteRoutedEventArgs e) => e.CanExecute = true;
    ...
    private static void OnPasswordChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        if (sender is PasswordBox passwordBox)
        {
            GetPasswordHandle(passwordBox).DoInternal(() =>
            {
    ...
    public static object GetPlaceholder(PasswordBox i) => i.GetValue(PlaceholderProperty);
    ...
    private static void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        {
            if (!box.CommandBindings.Any<CommandBinding>(j => j.Command == ClearCommand))
            {
            ...
            if (!box.CommandBindings.Any<CommandBinding>(j => j.Command == GenerateCommand))
            {
                box.CommandBindings.Add(new CommandBinding
                   (GenerateCommand,
            ...
            if (GetPasteCommand(box) is null)
                SetPasteCommand(box, new RelayCommand<PasswordBox>(i => i.Paste(), i => i is not null));

            OnActualPasswordChanged(box, new());
        }
    }

    private static void OnActualPasswordChanged(object sender, RoutedEventArgs e)
    {
        if (sender is PasswordBox box)
            GetPasswordHandle(box).DoInternal(() => SetPassword(box, box.Password));
    }
